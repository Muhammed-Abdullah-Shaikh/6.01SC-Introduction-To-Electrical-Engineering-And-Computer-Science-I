...
class MySMClass(sm.SM):
    def getNextValues(self, state, inp):
        sonars = inp.sonars
        front_distance = get_height(sonars[3], sonars[4]) if sonars[3] < 3 and sonars[4] < 3 else min(sonars[3], sonars[4])
        right_distance = get_height(sonars[6], sonars[7]) if sonars[6] < 3 and sonars[7] < 3 else min(sonars[6], sonars[7])
        if sonars[7] < 0.5 and sonars[7] > 0.3:
            right_distance = sonars[7]
        elif sonars[6] < 0.5 and sonars[6] > 0.3:
            right_distance = sonars[6]
        # right_distance = min(get_height(sonars[6], sonars[7]), sonars[7])
