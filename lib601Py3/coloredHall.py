# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/coloredHall.py
"""
...
import importlib
importlib.reload(dw)
possibleColors = ('black', 'white', 'red', 'green', 'blue', 'purple', 'orange', 'darkGreen', 'gold', 'chocolate', 'PapayaWhip', 'MidnightBlue', 'HotPink', 'chartreuse')
...
class TextInputSM(sm.SM):
    ...
    def getNextValues(self, state, inp):
        out = None
        first = True
        while out not in self.legalInputs:
            if not first:
                print('Illegal input:', out)
            out = input('Type an input (' + util.prettyString(self.legalInputs) + ' ): ')
            first = False
...
def wrapTextUI(m):
    ...
    """
    return sm.Cascade(sm.Cascade(TextInputSM(), m), sm.PureFunction(textOutput))


def wrapWindowUI(m, worldColors, legalInputs, windowName = 'Belief', initBelief = None):
    """
    ...
    ydim = 1
    window = DrawingWindow(dim * 50, ydim * 50 + 10, -0.2, dim + 0.2, -0.2, ydim + 0.2, windowName)
    drawWorld(dim)
    drawBelief(initBelief, window, dim)
    return sm.Cascade(sm.Cascade(sm.Cascade(TextInputSM(legalInputs), sm.PureFunction(processInput)), m), sm.PureFunction(processOutput))
...
def drawBelief(belief, window, numStates, drawNums = True):
    ...
    for x in range(numStates):
        window.drawRect((x + 0.2, y + 0.2), (x + 0.8, y + 0.8), color=colors.probToPyColor(belief.prob(x), unifP, (unifP + 0.3) / 1.3))
        if drawNums:
            window.drawText(x + 0.5, y + 0.5, '%3.2f' % belief.prob(x), color='white')
...
def makeSESwithGUI(worldSM, realColors, legalInputs, initBelief = None, verbose = False, title = 'hallway'):
    """
    ...
    """
    return wrapWindowUI(se.makeStateEstimationSimulation(worldSM, verbose), realColors, legalInputs, title, initBelief=initBelief)


def makeSim(hallwayColors, legalInputs, obsNoise, dynamics, transNoise, title = 'sim', initialDist = None):
    """
    ...
    worldSM = ssm.StochasticSM(initialDist, makeTransitionModel(dynamics, transNoise, n), makeObservationModel(hallwayColors, obsNoise))
    return makeSESwithGUI(worldSM, hallwayColors, legalInputs, verbose=True, title=title, initBelief=initialDist)