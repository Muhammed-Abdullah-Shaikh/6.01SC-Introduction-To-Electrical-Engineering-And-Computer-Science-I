# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/colors.py
"""
...
def probToPyColor(p, uniformP = 0.5, upperVal = None):
    ...
    if upperVal == None:
        upperVal = uniformP * 2
    if p > uniformP:
        return RGBToPyColor(HSVtoRGB(blueHue, 1.0, min(1.0, ((p - uniformP) / (upperVal - uniformP)) ** 0.5)))
    else:
        return RGBToPyColor(HSVtoRGB(redHue, 1.0, min(1.0, ((uniformP - p) / uniformP) ** 2)))
        return
...
def rootToPyColor(p, minV, maxV):
    ...
    """
    if p < 1:
        return RGBToPyColor(HSVtoRGB(greenHue, 1.0, 0.3 + 0.7 * (1 - (p - minV) / (1 - minV))))
    else:
        return RGBToPyColor(HSVtoRGB(redHue, 1.0, 0.2 + 0.8 * (1 - (maxV - p) / (maxV - 1))))
