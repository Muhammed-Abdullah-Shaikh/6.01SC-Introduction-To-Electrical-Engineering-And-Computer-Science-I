# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/gfx.py
from . import util
...
class PlotJob:
    ...
    def __init__(self, xname, yname, connectPoints, xfunc = None, yfunc = None, xbounds = 'auto', ybounds = 'auto'):
        self.xname = xname
        self.xfunc = xfunc
...
class RobotGraphics:
    ...
    def addStaticPlotFunction(self, x = ('step', None), y = ('step', None), connectPoints = connectPointsDef):
        """
        ...
        xname, xfunc = x
        yname, yfunc = y
        if yname in self.plotData:
            yfunc = None
        if xname in self.plotData:
            xfunc = None
        if xfunc or yfunc:
            self.plotJobs.append(PlotJob(xname, yname, connectPoints, xfunc, yfunc))
        if yfunc:
            self.plotData[yname] = []
        if xfunc:
            self.plotData[xname] = []
    ...
    def addStaticPlotSMProbe(self, x = ('step', None, None, None), y = ('step', None, None, None), connectPoints = connectPointsDef):
        """
    ...
    def addDynamicPlotSMProbe(self, y = ('step', None, None, None), connectPoints = connectPointsDef):
        """
    ...
    def setUpPlotting(self, dataProbes, plotTasks):
        ...
        for xname, xbounds, yname, ybounds in dataTasks:
            self.plotJobs.append(PlotJob(xname, yname, connectPoints, xbounds=xbounds, ybounds=ybounds))
    ...
    def addProbe(self, probe):
        ...
        if streamName not in self.plotData:
            ...
            sys.stderr.write('Trying to add multiple probes w/ name:' + streamName)
        self.traceTasks.append((machineName, mode, self.makeTraceFun(machineName, mode, valueFun, stream)))
    ...
    def doDataPlotJobs(self):
        ...
        for j in self.plotJobs:
            ...
            self.plotDataVersusData(xdata, j.xbounds, ydata, j.ybounds, j.yname + ' versus ' + j.xname, j.connectPoints)

        for j in self.plotJobs:
            self.plotData[j.xname] = []

    def plotDataVersusData(self, xData, xBounds, yData, yBounds, name, connectPoints, windowSize = None):
        if not windowSize:
        ...
        w = gw.GraphingWindow(windowSize, windowSize, xLower, xUpper, yLower, yUpper, name)
    ...
    def plotSlime(self):
        ...
        xLower, xUpper, yLower, yUpper = self.getEquallyScaledBounds(slimeX, slimeY)
        ...
        if self.slimeWindow:
            self.slimeWindow.destroy()
        self.slimeWindow = dw.DrawingWindow(self.windowSize, self.windowSize, xLower, xUpper, yLower, yUpper, 'slime')
        self.slimeWindow.drawText(xLower + 0.1 * xRange, yLower + 0.05 * yRange, util.prettyString(xLower), color='black')
        self.slimeWindow.drawText(xUpper - 0.1 * xRange, yLower + 0.05 * yRange, util.prettyString(xUpper), color='black')
        self.slimeWindow.drawText(xLower + 0.05 * xRange, yLower + 0.1 * yRange, util.prettyString(yLower), color='black')
        self.slimeWindow.drawText(xLower + 0.05 * xRange, yUpper - 0.1 * yRange, util.prettyString(yUpper), color='black')
        xMin = min(slimeX)
        xMax = max(slimeX)
        yMin = min(slimeY)
        yMax = max(slimeY)
        self.slimeWindow.drawLineSeg(xMin, yMin, xUpper - 0.1 * xRange, yMin, 'gray')
        self.slimeWindow.drawLineSeg(xMin, yMin, xMin, yUpper - 0.1 * yRange, 'gray')
        for i in range(maxT):
            self.slimeWindow.drawRobot(slimeX[i], slimeY[i], slimeX[i], slimeY[i], colors.RGBToPyColor(colors.HSVtoRGB(i * 360.0 / maxT, 1.0, 1.0)), 2)
