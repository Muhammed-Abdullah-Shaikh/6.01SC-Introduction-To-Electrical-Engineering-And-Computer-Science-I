# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/gridMap.py
"""
...
class GridMap:
    ...
    def __init__(self, xMin, xMax, yMin, yMax, gridSquareSize, windowWidth = defaultWindowWidth):
        """
    ...
    def makeWindow(self, windowWidth = defaultWindowWidth, title = 'Grid Map'):
        ...
        self.window = dw.DrawingWindow(int(windowWidth * dx / maxWorldDim), int(windowWidth * dy / maxWorldDim), self.xMin - margin, self.xMax + margin, self.yMin - margin, self.yMax + margin, title)
    ...
    def xToIndex(self, x):
        ...
        return util.clip(int(round((shiftedX - self.xMin) / self.xStep)), 0, self.xN - 1)
    ...
    def yToIndex(self, y):
        ...
        return util.clip(int(round((shiftedY - self.yMin) / self.yStep)), 0, self.yN - 1)
    ...
    def drawWorld(self):
        ...
        for xIndex in range(self.xN):
            for yIndex in range(self.yN):
                self.graphicsGrid[xIndex][yIndex] = self.drawNewSquare((xIndex, yIndex))
