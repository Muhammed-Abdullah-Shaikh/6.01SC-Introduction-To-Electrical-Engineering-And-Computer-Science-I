# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/optimize.py
"""
...
def optOverLine(objective, xmin, xmax, numXsteps, compare = operator.lt):
    """
    ...
    return argopt(objective, floatRange(xmin, xmax, (xmax - xmin) / float(numXsteps)), compare)


def optOverGrid(objective, xmin, xmax, numXsteps, ymin, ymax, numYsteps, compare = operator.lt):
    """
    ...
    """
    (score, y), x = optOverLine(lambda x: optOverLine(lambda y: objective(x, y), ymin, ymax, numYsteps, compare), xmin, xmax, numXsteps, lambda (s1, v1), (s2, v2): compare(s1, s2))
    return (score, (x, y))