# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/poly.py
"""
...
class Polynomial:
    ...
    def add(p1, p2):
        ...
        return Polynomial(vectorAdd(extend(p1.coeffs, resultLength), extend(p2.coeffs, resultLength)))
    ...
    def __str__(self, var = 'z'):
        if len(self.coeffs) == 0:
            ...
            return reduce(operator.add, [ ' + ' + prettyTerm(c, self.order - p, var) for c, p in zip(self.coeffs, list(range(self.order + 1))) if not c == 0.0 ], '')[3:]
...
def assertSameLength(a, b):
    ...
    """
    raise len(a) == len(b) or AssertionError('Error: lists must have same length' + str(a) + str(b))
