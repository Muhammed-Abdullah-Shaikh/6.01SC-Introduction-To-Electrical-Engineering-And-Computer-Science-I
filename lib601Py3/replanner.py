# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/replanner.py
"""
...
class Replanner(sm.SM):
    ...
    def getNextValues(self, state, inp):
        return newPathAndSubgoal(self.worldMap, inp, self.goalPoint, self.dynamicsModel, state, timeToReplanStaticMap)


def newPathAndSubgoal(worldMap, sensorInput, goalPoint, dynamicsModel, path, timeToReplan, scale = 1):
    """
    ...
    if timeToReplan(path, currentIndices, worldMap, goalIndices):
        if path:
            worldMap.undrawPath(path)

        def h(s):
            return scale * goalPoint.distance(worldMap.indicesToPoint(s))

        plan = ucSearch.smSearch(dynamicsModel, currentIndices, lambda s: s == goalIndices, heuristic=h, maxNodes=20000)
        if plan:
...
class ReplannerWithDynamicMap(sm.SM):
    ...
    def getNextValues(self, state, inp):
        ...
        return newPathAndSubgoal(map, sensors, self.goalPoint, dynamicsModel, state, timeToReplanDynamicMap, scale)
...
def timeToReplanDynamicMap(plan, currentIndices, map, goalIndices):
    ...
    plan, we failed).
    """
    return plan == None or planInvalidInMap(map, plan) or plan == [] and not goalIndices == currentIndices
...
def timeToReplanDynamicMapAndGoal(plan, currentIndices, map, goalIndices):
    ...
    """
    return plan == None or planInvalidInMap(map, plan) or plan == [] and not goalIndices == currentIndices or not plan[-1] == goalIndices
...
def timeToReplanDynamicMapWithKidnap(state, currentIndices, map, goalIndices):
    ...
    that is adjacent to the first one in the plan.
    """
    return state == None or not state == [] and not adjacent(currentIndices, state[0]) or planInvalidInMap(map, state) or state == [] and not goalIndices == currentIndices
...
class ReplannerWithDynamicMapAndGoal(sm.SM):
    ...
    def getNextValues(self, state, inp):
        ...
        if self.useCostDynamics:
            ...
            dynamicsModel = gridDynamics.GridDynamics(map)
        currentIndices = map.pointToIndices(sensors.odometry)
        path, subgoal = newPathAndSubgoal(map, sensors, goalIndices, dynamicsModel, state, timeToReplanDynamicMapAndGoal, scale)
