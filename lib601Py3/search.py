# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/search.py
"""
...
class SearchNode:
    ...
    def __repr__(self):
        if self.parent == None:
            ...
            return repr(self.parent) + '-' + str(self.action) + '->' + str(self.state)
            return
...
def search(initialState, goalTest, actions, successor, depthFirst = False, DP = True, maxNodes = 10000):
    """
    ...
    if goalTest(initialState):
        ...
        while not agenda.isEmpty() and maxNodes > count:
            ...
            for a in actions:
                ...
                if goalTest(newS):
                    ...
                    return newN.path()
                if newS in newStates:
                    pass
                elif not DP and n.inPath(newS) or DP and newS in visited:
                    pass
                else:
                    count += 1
                    if DP:
        ...
        if DP:
            print('Using DP', len(visited))
        return


def depthFirst(initialState, goalTest, actions, successor):
    """See C{search} documentation"""
    return search(initialState, goalTest, actions, successor, depthFirst=True, DP=False)


def breadthFirst(initialState, goalTest, actions, successor):
    """See C{search} documentation"""
    return search(initialState, goalTest, actions, successor, depthFirst=False, DP=False)


def depthFirstDP(initialState, goalTest, actions, successor):
    """See C{search} documentation"""
    return search(initialState, goalTest, actions, successor, depthFirst=True, DP=True)


def breadthFirstDP(initialState, goalTest, actions, successor):
    """See C{search} documentation"""
    return search(initialState, goalTest, actions, successor, depthFirst=False, DP=True)


def smSearch(smToSearch, initialState = None, goalTest = None, maxNodes = 10000, depthFirst = False, DP = True):
    """
    ...
    return search(initialState, goalTest, smToSearch.legalInputs, lambda s, a: smToSearch.getNextValues(s, a)[0], maxNodes=maxNodes, depthFirst=depthFirst, DP=DP)
