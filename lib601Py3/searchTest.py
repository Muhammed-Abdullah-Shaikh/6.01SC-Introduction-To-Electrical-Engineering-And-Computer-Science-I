# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/searchTest.py
from . import search
from . import util
from . import sm
from . import ucSearch
import importlib
importlib.reload(ucSearch)
import pdb
...
def mapDistTest(map, start, goal, searchFn = ucSearch.search, h = lambda x: 0):
    actions = list(range(max([ len(map[s]) for s in map ])))
...
def searchTestSM(goal):
    ...
    print('DepthFirstDP: ', end=' ')
    print(search.smSearch(NumberTestFiniteSM(goal, goal + 1), 1, depthFirst=True))
    print('DepthFirst: ', end=' ')
    print(search.smSearch(NumberTestFiniteSM(goal, goal + 1), 1, depthFirst=True, DP=False))
...
def eightTest(s, g, h):
    return ucSearch.smSearch(EightPuzzleSM(g), s, maxNodes=200000, heuristic=lambda s: h(s, g))
