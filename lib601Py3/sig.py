# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/sig.py
"""
...
class Signal:
    ...
    def plot(self, start = 0, end = 100, newWindow = 'Signal value versus time', color = 'blue', parent = None, ps = None, xminlabel = 0, xmaxlabel = 0, yOrigin = None):
        ...
        if newWindow == True or newWindow == False:
            ...
            title = newWindow
        if parent:
            w = gw.GraphingWindow(graphwidth, graphheight, start, end, minY - margin, maxY + margin, title, parent, xminlabel=xminlabel, xmaxlabel=xmaxlabel)
        else:
            if newWindow or Signal.__w == None:
                Signal.__w = gw.GraphingWindow(graphwidth, graphheight, start, end, minY - margin, maxY + margin, title, xminlabel=xminlabel, xmaxlabel=xmaxlabel)
            w = Signal.__w
    ...
    def crossings(self, n = None, z = None):
        ...
        return [ i for i in range(n - 1) if samples[i] > z and samples[i + 1] < z or samples[i] < z and samples[i + 1] > z ]
...
def polyR(s, p):
    ...
    """
    return util.sum([ c * Rn(s, k) for c, k in zip(p.coeffs, list(range(p.order, -1, -1))) ])
            ...
            for i in range(0, nsublen):
                ...
                if dx == 0:
                    ...
                    if not cubicinterpolation or k == 0 or k == nlen - 2:
                        val = (1 - dx) * samples[k] + dx * samples[k + 1]
                    else:
                        dxm = dx - 1
                        wm2 = -dxm * dxm * dx
                        wm1 = -wm2 - dxm
                        wp2 = dxm * dx * dx
                        wp1 = -wp2 + dx
                        val = wm2 * samples[k - 1] + wm1 * samples[k] + wp1 * samples[k + 1] + wp2 * samples[k + 2]
