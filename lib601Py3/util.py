# Embedded file name: /mit/6.01/mercurial/spring11/codeSandbox/lib601/util.py
"""
...
class Pose:
    ...
    def transform(self):
        ...
        return Transform([[cosTh, -sinTh, self.x], [sinTh, cosTh, self.y], [0, 0, 1]])
    ...
    def isNear(self, pose, distEps, angleEps):
        ...
        """
        return self.point().isNear(pose.point(), distEps) and nearAngle(self.theta, pose.theta, angleEps)
...
class Transform:
    ...
    def inverse(self):
        ...
        return Transform([[c, s, -c * x - s * y], [-s, c, s * x - c * y], [0, 0, 1]])
...
def prettyString(struct):
    ...
    if type(struct) == list:
        ...
        return '{' + ', '.join([ str(item) + ':' + prettyString(struct[item]) for item in struct ]) + '}'
...
def lineIndices(xxx_todo_changeme, xxx_todo_changeme1):
    ...
    raise type(i0) == int or AssertionError('Args to lineIndices must be pairs of integers')
    raise type(j0) == int or AssertionError('Args to lineIndices must be pairs of integers')
    raise type(i1) == int or AssertionError('Args to lineIndices must be pairs of integers')
    if not type(j1) == int:
